...
targeted_data <- targeted_data_raw |> 
  ...
  mutate(MetE_fmol_cell = targeted_data_raw$fmolProtein_Cell[targeted_data_raw$Protein == "MetE"]) |> 
  mutate(ACT1_fmol_cell = targeted_data_raw$fmolProtein_Cell[targeted_data_raw$Protein == "ACT1"]) |> 
  mutate(RBCL_fmol_cell = targeted_data_raw$fmolProtein_Cell[targeted_data_raw$Protein == "RBCL"]) |> 
...
# Total protein quota plot

# protquota_bp1 <- ggplot(targeted_data, aes(x=Temperature, 
#                                            y= fg_prot_cell, 
#                                            grouping = Treatment, 
#                                            fill = Temperature, 
#                                            alpha = B12)) + 
...
#                      labels = c( "+", "-")) +
#   labs(y= bquote('Total Protein (Femtograms cell'^-1*')') , 
#        x= "Treatment", 
#        fill='Temperature (°C)', 
...
metH_quota <- ggplot(
  ...
  labs(
    y = bquote('Femtomoles MetH cell' ^ -1) ,
    x = "Treatment",
    fill = 'Temperature (°C)',
    alpha = expression("B"[12])
  ) +
  guides(alpha = guide_legend(override.aes = list(
    fill = hcl(c(15, 195), 100, 0, alpha = c(.25, 1)), colour = NA
  ))) +
...
protquota_data <- read.csv("BCA1.1_05062019_a.txt_dilution25.csv")
...
# Werk the data 
protquota_data$temp <- factor(protquota_data$temp, levels = c("4", "12"))
protquota_data$B12 <- factor(protquota_data$B12, levels = c("+B12", "-B12"))

protquota_data$Treatment <- factor(paste(protquota_data$temp, ",", protquota_data$B12))

protquota_data$fg_prot_cell <- protquota_data$ug_cell_protein * 1e9
...
ggarrange(metH_quota, 
          ...
          font.label = list(size = 25, 
                            color = "black"))
# T-test to compare temps
x_prot4 <- protquota_data %>% filter(temp == "4")
x_prot4 <- x_prot4$fg_prot_cell

y_prot12 <- protquota_data %>% filter(temp == "12")
y_prot12 <- y_prot12$fg_prot_cell

t.test(x_prot4, y_prot12)


# Versions with transparent bg
...
x_methTemp <- targeted_data %>% filter(Temperature == "4")
x_methTemp <- x_methTemp$MetH_fmol_cell

y_methTemp <- targeted_data %>% filter(Temperature == "12")
y_methTemp <- y_methTemp$MetH_fmol_cell

t.test(x_methTemp, y_methTemp)


# B12 MetH - not sig (p-value = 0.5063)
x_methB12 <- targeted_data %>% filter(B12 == "+B12")
x_methB12 <- x_methB12$MetH_fmol_cell

y_methB12 <- targeted_data %>% filter(B12 == "-B12")
y_methB12 <- y_methB12$MetH_fmol_cell
...
x_meteTemp <- targeted_data %>% filter(Temperature == "4" & B12 == "-B12")
x_meteTemp <- x_meteTemp$MetE_fmol_cell

y_meteTemp <- targeted_data %>% filter(Temperature == "12" & B12 == "-B12")
y_meteTemp <- y_meteTemp$MetE_fmol_cell

t.test(x_meteTemp, y_meteTemp)


# B12 MetE - significantly less in noB12 samples (p-value = 0.001651)
x_meteB12 <- targeted_data %>% filter(B12 == "+B12")
x_meteB12 <- x_methB12$MetE_fmol_cell

y_meteB12 <- targeted_data %>% filter(B12 == "-B12")
y_meteB12 <- y_methB12$MetE_fmol_cell

t.test(x_methB12, y_methB12)


x_methB12_4 <- targeted_data %>% filter(B12 == "+B12" & Temperature == "4")
x_methB12_4 <- x_methB12$MetE_fmol_cell

y_methB12_ <- targeted_data %>% filter(B12 == "-B12")
y_methB12 <- y_methB12$MetE_fmol_cell

t.test(x_methB12, y_methB12)

# Break down by temps
x_meteB12_4 <- targeted_data %>% dplyr::filter(B12 == "+B12" & Temperature == "4")
x_metheB12_4 <- x_meteB12_4$MetE_fmol_cell

y_meteB12_4 <- targeted_data %>% filter(B12 == "-B12" & Temperature == "4")
y_meteB12_4 <- y_meteB12_4$MetE_fmol_cell

t.test(x_metheB12_4, y_meteB12_4)

# 12
x_meteB12_12 <- targeted_data %>% dplyr::filter(B12 == "+B12" & Temperature == "12")
x_meteB12_12 <- x_meteB12_12$MetE_fmol_cell

y_meteB12_12 <- targeted_data %>% filter(B12 == "-B12" & Temperature == "12")
y_meteB12_12 <- as.numeric(y_meteB12_12$MetE_fmol_cell)
